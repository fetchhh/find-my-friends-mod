...
public class NamePlateRender extends EntityRenderer<NamePlateEntity> {
    ...
    public void render(NamePlateEntity entity, float p_225623_2_, float p_225623_3_, PoseStack matrix, MultiBufferSource renderType, int ticks) {
        ...
        if (entity.showDist){
            String textContent = ClientFindConfig.textContent.get();
            name = Component.literal(Math.round(entity.dist) + textContent);
            ...
            int textColor = ClientFindConfig.textColor.get();
            fontrenderer.drawInBatch(name, f2, (float)i - 10, textColor, false, matrix4f, renderType, SEE_THROUGH, 0, ticks);
        }
